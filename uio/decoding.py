...
def temperature_sample(
    ...
    expanded_decodes, expanded_log_prob, expanded_all_logprob = _temperature_sample_single_trial(
        expanded_inputs,
        expanded_cache,
        tokens_to_logits,
        eos_id,
    ...
    return jnp.take_along_axis(
        ...
        log_prob, jnp.squeeze(idxs, axis=-1), axis=-1), jnp.take_along_axis(all_logprob, idxs, axis=1)
...
def _temperature_sample_single_trial(
    ...
    def sampling_loop_body_fn(state: SamplingLoopState) -> SamplingLoopState:
        ...
        one_hot_indices = jax.nn.one_hot(i, all_log_prob.shape[-1], dtype=next_log_prob.dtype)
        all_log_prob = all_log_prob + one_hot_indices * jnp.expand_dims(next_log_prob, -1)
...
def beam_search(inputs: jnp.ndarray,
    ...
    def beam_search_loop_body_fn(state: BeamState) -> BeamState:
        ...
        # Call fast-decoder model on current tokens to get next-position logits.
        # --> [batch * beam, vocab]
        flat_logits, new_flat_cache = tokens_to_logits(flat_ids, flat_cache, state.cur_index, state.live_seqs)
        ...
        one_hot_indices = jax.nn.one_hot(state.cur_index, state.all_logprobs.shape[2], dtype=state.all_logprobs.dtype)

        # curr_log_probs = top_alive_log_probs - state.live_logprobs
        def identity_fn(x): return x
        def update_fn(x, y): return x - y
