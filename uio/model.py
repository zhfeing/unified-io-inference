...
class UnifiedIOModel(nn.Module):
        ...
        for i in range(n_groups):
            ...
            if text_answer_options:
                soft_targets = common_utils.onehot(
                    text_decoder_targets, text_logits.shape[-1],
    ...
    def predict_batch_with_aux(
        ...
        encoded_inputs, encoder_masks = self.module.apply(
            {'params': params},
            text_encoder_inputs, image_encoder_inputs, text_encoder_masks, image_input_masks,
            image_encoder_pos_ids=batch.get('image_encoder_pos_ids', None),
            text_encoder_pos_ids=batch.get('text_encoder_pos_ids', None),
            enable_dropout=False, method=self.module.encode)
